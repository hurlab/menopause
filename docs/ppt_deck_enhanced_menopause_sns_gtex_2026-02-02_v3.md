...
## GTEx “batch” reality: sequencing center and batches are major covariates

- In GTEx adipose, technical variables are not independent of biology:
  - **SMCENTER** (sequencing center) differs by **depot**, **sex**, and **age-bin distribution**.
  - “Batch correction” is not a single plug-and-play step because aggressive removal can also remove biology if batch is confounded with group.
- Therefore in downstream models we:
  - Always include **SMCENTER** as a covariate.
  - Treat “gene-expression batch” (e.g., **SMGEBTCH**) as a **sensitivity/robustness** adjustment (not a single “best” model).

![](GTEx_v10_AT_analysis_out/figs/institution_batch_panel.png){width=95%}

---

## Why we don’t just “ComBat everything”: batch adjustment changes conclusions

- Example (female subcutaneous; 40–49 vs 50–59): naive DE can yield “significant” genes that disappear after adding **SMCENTER** (likely false positives).
- Visualization: left shows naive vs center-adjusted DE comparison; right shows PCA before/after ComBat (illustrates how strong center-related structure is).

![](GTEx_v10_AT_analysis_out/figs/batch_correction_summary_panel.png){width=49%}
![](GTEx_v10_AT_analysis_out/figs/PCA_combat_correction_comparison.png){width=49%}
